...
def filter_pair(pair, src_max_len, tgt_max_len):
    ...
    """
    # in python3, map is not subsriptable, so we need to convert it to a list 
    # we have convert is to a list to get its list
    # get length 
    ...
    with open(path) as fin:
        for line in fin:
            try:
                ...
                # map iterator can only use once, so here we need to convert it to a list
                # size = [len(v) for v in pair] => now we can not iterates pair the second time
                pair = list(pair)
